...
首页的“热点”板块读取 `data/news.json`，展示 AI、美股、A股三个主题的每日推荐，并可按日期回看近 7 个自然日的历史记录。
...
脚本会并行抓取多个查询与来源，校验响应确实为含条目的 RSS/Atom，再去重并为每个主题保留最多 6 条。每次运行都会记录检查时间、来源状态与主题新鲜度；抓取失败时保留上一次数据，并在页面明确标注“沿用旧结果”。新闻按 Asia/Shanghai 日期每天保存一份快照，同一天重复更新会替换当日快照，超过近 7 个自然日的记录会自动清理。工作流最后会校验 AI 主题本次是否取得新结果，避免静默成功。
...
也可以启动仅绑定本机回环地址的预览服务，再打开终端输出的网址；展览页会显示“使用本机登录态刷新”按钮：

```powershell
node scripts/serve-exhibitions-local.mjs
```

一键入口只接受来自同端口 `localhost/127.0.0.1`、带专用校验头的 POST 请求，不会部署为公开 API。刷新成功前使用临时文件原子替换；未登录、OpenCLI 不可用、超时或没有结果时命令会返回失败并完整保留上一份线索。该步骤依赖本机浏览器登录态，不放进 GitHub Actions，也不会把 Cookie 写入仓库。

前端会把线索采集日期与主展会数据日期比较：线索更早或超过 7 天时明确标记“已过期”，但仍可作为发现参考。展会事实数据以“上海市会展业公共信息服务平台”的已备案项目为主库，并结合 `data/exhibitions-curated.json` 中的上海市商务委重点展会排期与近期官方发布；采集器会补充备案状态、文号、主办方、展览面积、展会类型和同期展会，并探测国家会展中心、上海新国际博览中心、上海世博展览馆等场馆排期。`.github/workflows/update-exhibitions.yml` 每日更新上海排期，同时从公开聚合页补充待官网复核的长尾线索；既有的未来全球精选会继续保留，避免上海日更误删其他地区内容。

## 漫展专区

`conventions.html` 是 Exhibit Atlas 的漫展专区，默认把嘉宾、出席日期和签售安排放在活动日期与票务之前。页面读取 `data/conventions.json`，支持按嘉宾、漫展、省份和城市搜索，按省份—城市二级筛选，也可手输城市进行模糊匹配，并可只看“嘉宾已公布”“嘉宾待官宣”或当前设备关注的活动。
